...
### OAuth / Social Login Binding Must Verify Initiator, Not Landing Session

In any third-party login/binding callback (LINE, Google, FB…), **never** decide which site account to bind using only the landing browser's current session (`wp_validate_auth_cookie()` / `get_current_user_id()`). A callback is a no-nonce top-level GET — a `SameSite=Lax` `logged_in` cookie is sent when a victim clicks a crafted link, so an attacker's third-party account gets bound to the victim (account takeover). Sign the **initiator's** uid into the HMAC-protected OAuth `state` at `/login`, and only bind when `initiator === current logged-in user`.

```php
// ❌ WRONG — trusts whoever is logged in at the landing browser → binding CSRF / takeover
$uid = wp_validate_auth_cookie( '', 'logged_in' );
if ( $uid ) {
    update_user_meta( $uid, 'line_user_id', $third_party_id ); // Bound to the current cookie's owner.
}

// ✅ CORRECT — gate binding on the signed initiator
$initiator_id = get_initiator_from_state( $state ); // Extracted from verified, HMAC-signed state.
$current_id   = wp_validate_auth_cookie( '', 'logged_in' );
if ( 0 < $initiator_id && $initiator_id === $current_id ) {
    update_user_meta( $current_id, 'line_user_id', $third_party_id ); // Owner binding in own browser.
} else {
    // initiator 0 → ignore landing cookie, log in as third-party identity.
    // initiator ≠ current → suspicious, do not bind; fall back to third-party login.
}
```

Carry the initiator through `state` (survives iOS / in-app cross-browser hand-off), **not** a cookie. `sanitize_*()` does not help here — this is an authorization-logic flaw, not an input-sanitization one.

Origin: distilled from `woocommerce-notify` LINE-login OAuth binding CSRF fix (2026-07-31).
...
- [ ] OAuth/social-login callback binds on signed initiator, not landing session
