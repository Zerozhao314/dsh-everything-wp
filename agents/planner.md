...
2. **No web search** — use only user-provided URLs (`WebFetch`) and `@skill-name` references from `skills/`.
...
|--------------------|-----------------------------------------------------------------------------|
| `@skill-name`      | Load `skills/<name>/SKILL.md` for API patterns and best practices.          |
...
skill docs from `skills/` — do not wait for the user to
mention them:
...
**Read `rules/acceptance-criteria.md` before writing this layer** — it is the
contract for how criteria are specified and how they flow into implementation and review.

For each operation step that needs detail, write a user story followed by **BDD acceptance
criteria in `Given / When / Then` scenarios** (not vague bullets):
...
**Scenario**: <outcome-focused title>
  ...
  **Then** <observable, checkable result>
   (**And** <extra Given / Then clauses as needed>)

**Scenario**: <the rejection / unhappy path>
  **Given** ...
  **When** ...
  **Then** ...
```

Scenario rules (full detail in `rules/acceptance-criteria.md`):

- **Observable, not internal** — assert on what a user or API caller can see, not private methods.
- **Concrete data** — real example values (`Given an order with total 0`) over abstractions.
- **One behavior per scenario** — two `When`s means two scenarios.
- **Always include the unhappy paths** — for anything touching input, auth, or money, write the
  rejection scenarios (missing nonce → 403, wrong capability → `wp_die`, empty field → validation
  error). These are what catch real bugs, and they become the guard tests in `/wp-todo`.

Each scenario must be concrete enough to become a PHPUnit test without a follow-up question —
in TDD mode, `task-executor` turns each scenario into at least one test.

### Step 5: Layer 3 — Development Tasks

Break each user story into concrete tasks. For each task:

1. **Decide if a `commands/` command applies** — `Glob` the commands directory, then match by task semantics
2. **Mark with `→ /wp-command-name`** if applicable, otherwise leave plain (no `(manual)` suffix needed — absence of `→` means manual)
3. **Group by layer**: Data / API / Interface / Integration

#### Risk tier per area

Tag each area with a review risk tier (🟢 / 🟡 / 🔴) from
`rules/acceptance-criteria.md §3` so `code-reviewer` knows how deep to look.
Decide from blast radius / reversibility / independence of verification:

- 🔴 **Red** — the area touches auth, capabilities, `$wpdb`, payments/checkout, file uploads,
  or REST endpoints accepting input. Never downgrade.
- 🟡 **Yellow** — new feature or refactor of existing logic, data-flow or schema changes.
- 🟢 **Green** — pure glue (menu registration, asset enqueue), low-risk internal tooling.

Put the tier on a `**Risk Tier**:` line at the top of each area file.
...
```
Next step: `/wp-todo spec/booking-system/customer-flow.md --tdd=int`
```
...
**Next step**: `/wp-todo spec/<feature-name>/01-<file>.md [--tdd=...]`
...
**Risk Tier**: 🟡 Yellow — Selective Review  <!-- 🟢 / 🟡 / 🔴 per rules/acceptance-criteria.md §3 -->

## User Stories
<US-1, US-2, ... from Step 4 — each with its Given/When/Then acceptance scenarios>

## Development Tasks

### Data Layer
- [ ] <task> → `/wp-custom-table`
- [ ] <task>
...
> Tasks with `→ /wp-command` have available automation in `commands/`.
...
Use this for human exploratory testing after `/wp-todo` completes — automated test coverage comes from TDD mode (if recommended) or from `/wp-test-generate` for legacy code.
```
