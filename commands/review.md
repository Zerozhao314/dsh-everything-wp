...
Reviews only files changed since the last commit. It first **classifies the diff's risk tier**
(🟢 / 🟡 / 🔴, per `rules/acceptance-criteria.md`) to decide how deep to look —
🔴 auth/capability/SQL/payment changes get line-by-line scrutiny, 🟢 glue code gets a skim —
then reports findings in six areas:

1. **Acceptance Criteria** — do the diff and its tests satisfy the spec's `Given/When/Then`
   scenarios, including the rejection/unhappy paths?
2. **Security** — nonce, sanitization, capability, SQL prepare, output escaping, file ops
3. **Performance** — N+1 queries, missing cache, hook overload, autoload bombing
4. **Simplification** — duplication, dead code, debug residue, reinventing WP core
5. **Test Coverage Gap** — new behavior without a corresponding test
6. **i18n** — hardcoded strings, missing text domain, dynamic `__($var)`, escape + i18n combos
...
After the report, the agent checks each generalizable 🔴/🟡 finding against `rules/wp-essentials.md`:
...
Located at: `agents/code-reviewer.md`
